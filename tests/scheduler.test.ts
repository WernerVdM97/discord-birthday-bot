...
describe("checkAndPostBirthdays", () => {
	it("posts wish with tag emoji when there is a birthday today", async () => {
		// Set date to June 14
		...
		await checkAndPostBirthdays(
			client as unknown as Parameters<typeof checkAndPostBirthdays>[0],
		);

		expect(sendMock).toHaveBeenCalledWith(expect.stringContaining("<@u1>"));
